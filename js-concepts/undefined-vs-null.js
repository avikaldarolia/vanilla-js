/**
 * In JavaScript, "undefined" and "null" are both primitive datatypes representing the absence of a value. However, "undefined" is used to indicate an uninitialized variable or a non-existent property. On the other hand, "null" is used by developers to indicate that a variable intentionally has no value.

Consider the following example: Here we return an object if id === 1, when id === 2 we return null to indicate user doesn't exist for other id javascript will return undefined as it is not handled by developer.
 */
